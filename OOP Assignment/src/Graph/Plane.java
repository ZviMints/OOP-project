...
/*
    ...
    Copyright (C) 2012 Rafael Rendֳ³n Pablo <smart.rendon@gmail.com>
  ...
  public void paintComponent(Graphics g)
  {
    ...
    for (Function f : functionList) {
      ...
      for (x0 = middle; x0 - dx >= start; x0 -= dx) {
        ...
        //// [LEFT]
        if(f.derivativeAtX(x0-EPS)*f.derivativeAtX(x0+EPS)<0)
        {
        	g2d.setColor(Color.YELLOW);
        	g2d.drawOval(ix(x0), iy(y0), 3, 3);
        }
        else
        	g2d.setColor(f.getColor());
        
        g2d.drawLine(ix(x0), iy(y0), ix(x0), iy(y1)); // Draw function on left 
      }
      ...
      for (x0 = middle; x0 + dx <= end; x0 += dx) {
        ...
        g2d.drawLine(ix(x0), iy(y0), ix(x0), iy(y1)); // Draw function on right 
      }
  ...
  public void drawAxis(Graphics2D g2d)
  {
    ...
    for (double i = startX; i <= right; i += gridIntervalX) {
      if (ix(i) == ix(0)) {
        ...
        g2d.drawLine(ix(i) + 7, iy(top) + 7, ix(i), iy(top));
        g2d.drawString("Y PLOT", ix(i) - 55, iy(top) + 15);
    ...
    for (double i = startY; i >= bottom; i -= gridIntervalY) {
      if (iy(i) == iy(0)) {
        ...
        g2d.drawLine(ix(right) - 7, iy(i) + 7, ix(right), iy(i));
        g2d.drawString("X PLOT", ix(right) - 45, iy(i) - 10);
      } else {
