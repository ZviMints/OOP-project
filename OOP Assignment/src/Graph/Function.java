...
public class Function {
  ...
  public Function(Polynom_able pol, String name)
  {
	String definition = pol.toString();
    parser = new Parser(definition);
    setDefinition(definition);
    setActive(true);
    setName(name);
    this.pol = (Polynom) pol;
  }
  ...
  public Color getColor()
  {
	...
	public double derivativeAtX(double x) {
		Polynom pol_afterDervicative = (Polynom) pol.derivative();
		return pol_afterDervicative.f(x);
	}
  
	public double rootPoint(double x0, double x1, double EPS) {
		return pol.root(x0, x1, EPS);
	}
